...
export const finishClicked = () => {
  const val = new Date(DateTime.now('en', DateTime.browserTimeZone()))

  /* needs i18n */
  const formatted =
    `${val.getMonth() + 1}/${val.getDate()}/${val.getFullYear()} ${val.getHours()}:${val.getMinutes()}`

  return (dispatch) => {
    dispatch({ type: types.FINISH_CLICKED, fileName:  `[Untitled ${formatted}]`})
  }
}
