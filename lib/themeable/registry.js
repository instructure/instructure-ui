...
const getRegistry = function () {
  let registry = canUseDOM ? window.GLOBAL_THEME_REGISTRY : GLOBAL_THEME_REGISTRY

  if (!registry) {
    registry = GLOBAL_THEME_REGISTRY = DEFAULT_REGISTRY
    if (canUseDOM) {
      window.GLOBAL_THEME_REGISTRY = registry
    }
  }
