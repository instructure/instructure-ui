      ...
      return (
        <div>
          ...
          <Container padding="small">
            <NumberInput label={label} step="0.1" min="0.1" locale={this.state.locale} defaultValue="2.4"/>
          </Container>
        </div>
      )
    }
...
class NumberInput extends Component {
  ...
  handleKeyDown = (event) => {
    ...
    if (dir) {
      ...
      const newValue = this.applyStep(dir)
      this._input.value = formatNumberForLocale(newValue, this.locale)
  ...
  handleClickUp = (event) => {
    event.preventDefault()
    const newValue = this.applyStep(1)
    this._input.focus()
    this._input.value = formatNumberForLocale(newValue, this.locale)
    this.props.onChange(event)
  };
