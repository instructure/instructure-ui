...
class AutocompleteField extends Component {
  ...
  renderStaticOption (message) {
    return (
      <li // eslint-disable-line
        ...
        onClick={this.props.onStaticClick}
      >
        {message}
      </li>
    )
  ...
  renderOptions () {
    ...
    return this.props.options.map((option, index) => {
      ...
      const selected = getOptionId(this.props.selectedOption) === id
      const handlers = {
        onMouseEnter: () => this.highlightOption(index),
        onClick: (event) => this.select(event, option)
      }
  ...
  render () {
    ...
    const highlightedOption = this.props.options[this.state.highlightedIndex]
