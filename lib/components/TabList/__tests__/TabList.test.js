...
describe('TabList', function () {
  ...
  it('should render the correct number of panels', function () {
    ...
    const children = testbed.dom.findAll(TabPanel)
  ...
  it('should default to selectedIndex being 0', function () {
    ...
    expect(panels.nodes[0].getAttribute('aria-hidden')).to.equal('false')
  })

  it('should honor the selectedIndex prop', function () {
    testbed.render({
      selectedIndex: 1
    })
    ...
    expect(panels.nodes[1].getAttribute('aria-hidden')).to.equal('false')
  })
  ...
  it('should not allow selecting a disabled tab', function () {
    ...
    expect(tabs.nodes[2].getAttribute('aria-selected')).to.equal('false')
  ...
  it('should call onChange when selection changes', function () {
    ...
    testbed.dom.findAll(Tab).nodes[1].click()
  ...
  it('should update the selected tab via keyboard arrow keys', function () {
    ...
    testbed.dom.find('[aria-selected="true"]').keyDown(37)

    expect(tabs.nodes[1].getAttribute('aria-selected')).to.equal('true')
    expect(panels.nodes[1].getAttribute('aria-hidden')).to.equal('false')

    testbed.dom.find('[aria-selected="true"]').keyDown(39)
    testbed.dom.find('[aria-selected="true"]').keyDown(40)
  ...
  it('should not change selectedIndex when clicking a disabled tab', function () {
    ...
    testbed.dom.findAll(Tab).nodes[2].click()
