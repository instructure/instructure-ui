...
describe('<TabList />', function () {
  ...
  it('should preserve TabPanel keys', function () {
    const subject = testbed.render({
      children: <TabPanel title="foobar" key="foo" />
    })

    expect(subject.first(TabPanel).getKey()).to.equal('foo')
    ...
    expect(subject.first(TabPanel).getKey()).to.equal('foo')
    expect(subject.last(TabPanel).getKey()).to.equal('bar')
  })
  ...
  it('should focus the defaultSelectedIndex tab when focus is set', function () {
    const subject = testbed.render({
      defaultSelectedIndex: 1,
      focus: true
    })
  ...
  it('should focus the selectedIndex tab when focus is set', function () {
    const subject = testbed.render({
      selectedIndex: 1,
      focus: true
    })

    const tabs = (n) => subject.find(':dom[role="tab"]')[n]

    expect(document.activeElement).to.equal(tabs(1))

    subject.props({ selectedIndex: 0 })
  ...
  it('should meet a11y standards when set to the simple variant', function (done) {
    const subject = testbed.render({
      variant: 'simple'
    })
  ...
  it('should meet a11y standards when set to the accordion variant', function (done) {
    const subject = testbed.render({
      variant: 'accordion'
    })
  ...
  it('should meet a11y standards when set to the minimal variant', function (done) {
    const subject = testbed.render({
      variant: 'minimal'
    })
