  ...
  describe('with the default theme', function () {
    ...
    describe('simple variant', function () {
      it('should ensure text and tab background meet 3:1 contrast', function () {
        expect(contrast(variables.simple.selected.color, variables.simple.selected.background))
          .to.be.above(3)
      })
    })
    ...
    describe('accordion variant', function () {
      it('should ensure text and tab background meet 3:1 contrast', function () {
        expect(contrast(variables.accordion.color, variables.accordion.background))
          .to.be.above(3)
        expect(contrast(variables.accordion.color, variables.accordion.hover.background))
          .to.be.above(3)
        expect(contrast(variables.accordion.selected.color, variables.accordion.selected.background))
          .to.be.above(3)
      })
    })
  })
