...
describe('<CheckboxGroup />', function () {
  ...
  it('displays error messages inside the legend', function () {
    ...
    expect(subject.find('legend:contains("Error Message")').dom())
      .to.exist
  })
  ...
  it('does not call the onChange prop when disabled', function () {
    ...
    subject.find('input[value="football"]').trigger('change', {
      target: {
        checked: true,
        value: 'football'
      }
  ...
  it('should not update the value when the value prop is set', function () {
    ...
    expect(subject.unwrap().value).to.deep.equal(['tester'])
  })
  ...
  it('should add the checkbox value to the value list when it is checked', function () {
    ...
    expect(subject.unwrap().value).to.deep.equal(['football'])
  })
  ...
  it('should check the checkboxes based on the defaultValue prop', function () {
    ...
    expect(subject.find('input[value="football"]').dom().checked)
      .to.be.true

    expect(subject.find('input[value="volleyball"]').dom().checked)
      .to.be.true
  })
  ...
  it('should remove the checkbox value from the value list when it is unchecked', function () {
    ...
    expect(subject.unwrap().value).to.deep.equal(['basketball', 'volleyball'])
  })
