...
/**
  Use Badge to communicate a number of unread or incomplete
  items (`type="count"`) or simply that there is new content or an
  update that requires the user's attention (`type="notification"`).

  Note the use of the `pulse` prop to make the Badge slowly pulse twice on mount.

  ```jsx_example
    <div>
      <Badge count={2} margin="0 medium 0 0" pulse>
        <Button variant="icon">
          <PlaceholderIcon title="Edit page" />
        </Button>
      </Badge>
      <Badge type="notification" pulse>
        <Button variant="icon">
          <PlaceholderIcon title="Edit page" />
        </Button>
      </Badge>
...
class Badge extends Component {
  ...
  renderCount () {
    const {count, countUntil, formatOverflowText} = this.props
    const overflowText = formatOverflowText(count, countUntil)

    if (this.countOverflow()) {
      return overflowText
    } else {
      return count
    }
  ...
  renderBadge () {
    ...
    return (
      <Container
        ...
        id={(type === 'count' && !standalone) ? this._defaultId : null}
      >
        {(type === 'count') ? this.renderCount() : null}
      </Container>
    )
  }
