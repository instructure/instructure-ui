...
export default class Alert extends Component {
  ...
  handleTimeout = () => {
    if (this.props.timeout > 0) {
      this._timeouts.push(setTimeout(() => { this.close() }, this.props.timeout))
    }
  }
  ...
  close = () => {
    ...
    this.setState({ open: false }, () => {
      if (this.props.onClose && this.props.transition === 'none') {
        this.props.onClose()
      }
    })
  }
  ...
  handleKeyUp = (event) => {
    if (this.props.dismissible && event.keyCode === keycode.codes.esc) {
      this.close()
    }
  }
