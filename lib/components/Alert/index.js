...
<div>
  ...
  <Alert
    ...
    margin="small"
  >
    Sample warning text. This alert is not dismissable and cannot be closed.
  </Alert>
</div>
```
...
export default class Alert extends Component {
  static propTypes = {
    ...
    */
    ...
    /**
    * Close button label. Required if `dismissable` is true
    */
    ...
    */
    ...
    /**
    * if isOpen transitions from truthy to falsey, it's a signal to close and unmount the alert.
  ...
  constructor (props) {
    super(props)
    invariant(!props.dismissable ||
              (props.dismissable && props.closeButtonLabel),
              'If dismissable is true, then closeButtonLabel must also be provided')
  ...
  handleKey = (event) => {
    if (this.props.dismissable && event.keyCode === 27) {
      this.close()
    }
  }
  ...
  // copied from ../../util/dismissable.js
  // we cannot use dismissable directly, because it adds the close button to children,
  ...
  render () {
    if (this.props.transitionType === 'none') {
      return this.state.isOpen ? this.renderAlert() : null
    }
    return (
      <Transition
