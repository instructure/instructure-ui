...
/**
  ...
  `<ContentBox/>` defaults to no padding around its content. To add padding, use the `padding` prop.

  Use the `textAlign` prop to change the alignment of the text inside `<ContentBox />`. In use cases
  where `<ContentBox/>` is not absolutely positioned, use the `margin` prop to set margin around
  the component.
  ...
  <div>
    ...
    <ContextBox
      ...
      placement="left">
      <Heading level="h3">Hello World</Heading>
      <Typography size="small">This ContentBox is right-text-aligned</Typography>
    </ContextBox>
    <ContextBox
      placement="bottom"
      padding="medium"
    ...
    >
      <Typography size="small">
        This ContentBox uses the inverse variant and medium padding. Its size prop is set to small,
        which causes long strings like this to wrap. It also has top margin to separate it from
        the ContentBoxes about it.
      </Typography>
    </ContextBox>
  </div>
  ```
