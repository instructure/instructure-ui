...
describe('<TreeBrowser />', function () {
  ...
  it('should render all collections at top level if showRootCollection is true and rootId is undefined', function () {
    const tree = testbed.render({rootId: undefined})
    expect(tree.find('button').length).to.equal(4)
  })

  describe('expanded', function () {
    it('does not set state without defaultExpanded prop', function () {
      const tree = testbed.render()
      expect(tree.instance().state).not.to.be_present
    })

    it('accepts an array of default expanded collections and sets state', function () {
    ...
    it('updates expanded state on handleCollectionClick', function () {
      const tree = testbed.render()
      tree.find('button').simulate('click')
      expect(tree.instance().state.expanded).to.include(2)
    })

    it('calls onCollectionClick when expanding and collapsing', function () {
    ...
    it('persists the state of expanded children when parent collapsed', function () {
      const tree = testbed.render()
    ...
    it('should not update expanded on click when set as explicit prop', function () {
  ...
  describe('collections', function () {
    ...
    it('renders icons properly', function () {
      const tree = testbed.render({
  ...
  describe('for a11y', function () {
    ...
    it('updates tabindex on keyboard nav', function () {
