...
/**
  ...
  <div>
    <div style={{width: 200}}>
      <Button fluidWidth margin="0 0 large 0">{lorem.sentence()}</Button>
    </div>
    <div style={{width: 600}}>
      <Button fluidWidth href="example.html">{lorem.sentence()}</Button>
    </div>
  </div>
  ```
**/
...
class Button extends Component {
  ...
  handleKeyDown = (e) => {
    ...
    if (this.elementType !== 'button' &&
      ...
      if (typeof onClick === 'function' && !disabled) {
        ...
        ReactDOM.findDOMNode(this._button).click() // eslint-disable-line react/no-find-dom-node
      }
    }
  }
  ...
  focus () {
    ReactDOM.findDOMNode(this._button).focus() // eslint-disable-line react/no-find-dom-node
  }
