...
/**
  ...
  function Example () {
    ...
    return (
      <RadioInputGroup onChange={handleChange} name="example" defaultValue="foo" description="Select something">
  ...
  <Example />
  ```
  With the `layout` prop set to `inline` to render the description in its own column at screens `tablet` size
  ...
  <Example />
  ```
  With the `layout` property set to `columns` the inputs will render horizontally at screens `tablet` size
  and up (see [Grid](#Grid)).
  ```js_example
  function Example () {
    const inputs = [
...
export default class RadioInputGroup extends Component {
  ...
  renderChildren () {
    ...
    if (variant === 'toggle') {
      return <div className={styles.toggle}>{fields}</div>
    } else {
      return fields
    }
  }
  ...
  render () {
    ...
    return (
      <FormFieldGroup
        ...
        // Note: the 'toggle' variant can't support 'columns' layout so we have to force it to 'stacked'
        // TODO: split out toggle variant into its own component
        layout={(variant === 'toggle') ? ((layout === 'columns') ? 'stacked' : layout) : layout}
