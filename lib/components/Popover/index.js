    ...
    return (
      <Container padding="large 0">
        <Popover
          ...
          defaultFocusElement={() => this._username}
  ...
  renderContent () {
    const content = (
      <ContextBox {...pickProps(this.props, ContextBox.propTypes)} elementRef={this.props.contentRef}>
        {pick(Popover.Content, this.props.children)}
      </ContextBox>
    )
    ...
    if (this.shown) {
      return (
        <Dialog
          ...
          closeButtonPlacement={this.closeButtonPlacement}
          closeButtonVariant={this.props.variant === 'inverse' ? 'icon-inverse' : 'icon'}
        >
          {content}
        </Dialog>
      )
    } else if (this.props.shouldRenderOffscreen) {
      return content
    } else {
      return null
    }
