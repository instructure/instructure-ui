...
export default class PopoverMenu extends Component {
  ...
  focusIfNeeded = () => {
    if (!this.show && this.props.focusTriggerOnClose) {
      this.focus()
    }
  }

  setShow (show, callback) {
    const cb = () => {
  ...
  handleToggle = () => {
    this.setShow(!this.show, this.props.onToggle)
  }

  handleClose = (e) => {
    this.setShow(false, this.props.onClose)
  }

  handleSelect = (e) => {
    this.setShow(false, this.props.onSelect)
  }
