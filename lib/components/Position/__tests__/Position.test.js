...
describe('<Position />', function () {
      ...
      return (
        <div style={{
          ...
          <Position {...props}>
            ...
            <PositionContent>
              <ContextBox placement={this.props.placement} ref={(el) => { this._content = el }}>
                <div>Content</div>
              </ContextBox>
            </PositionContent>
          </Position>
  ...
  it('should render `bottom` placement by default', function () {
    const subject = testbed.render()
    testbed.tick() // portal

    const { position } = findAll(subject)
    expect(position.prop('placement')).to.equal('bottom')
  })
  ...
  describe('with offset props', function () {
    ...
    assertOffset('top', 10, (contentRect, top, left) => {
      expect(contentRect.top).to.equal(top + 10)
      expect(contentRect.left).to.equal(left + 10)
    })

    assertOffset('left', 10, (contentRect, top, left) => {
      expect(contentRect.top).to.equal(top + 10)
      expect(contentRect.left).to.equal(left + 10)
    })

    assertOffset('right', 10, (contentRect, top, left) => {
      expect(contentRect.top).to.equal(top + 10)
      expect(contentRect.left).to.equal(left + 10)
    })
