...
class Position extends Component {
  ...
  static propTypes = {
    ...
    /**
     * Callback fired when `<Position />` content has been mounted and is positioned
     */
  ...
  getContainer = () => {
    return this.props.mountNode || this.state.container || document.body
  }

  position = callback => {
    const { style, placement, container } = calculateElementPosition(this._content, this._target, {
  ...
  renderContent () {
    ...
    if (content && React.Children.count(content.props.children) > 0) {
      ...
      content = (
        <Portal open onOpen={this.handlePortalOpen} mountNode={this.getContainer}>
          {content}
        </Portal>
      )
    }
