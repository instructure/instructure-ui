...
import getAbsoluteBoundingClientRect from '../../util/getAbsoluteBoundingClientRect'
...
class Position extends Component {
  ...
  handlePortalReady = () => {
    if (!this._cancelTracking) {
      // track changes in target position
      this._cancelTracking = trackPosition(this._target, this.updatePosition)
    }
    this.updatePosition()
  }
  ...
  updatePosition = (lastTargetRect) => {
    ...
    if (!this._isUnmounted && this._content && this._target) {
      const targetRect = lastTargetRect || getAbsoluteBoundingClientRect(this._target) || {}
      const contentRect = this._contentRect || getAbsoluteBoundingClientRect(this._content) || {}
  ...
  render () {
    ...
    if (content) {
      content = safeCloneElement(content, {
        ...
        ref: (el) => {
          this._content = el
          this._contentRect = getAbsoluteBoundingClientRect(this._content)
        },
        style: {
