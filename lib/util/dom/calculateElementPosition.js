...
function _calculatePosition (element, target, options) {
  ...
  return adjustForPositionedParents(element, adjustForScrollParents(element, position))
}
...
function adjustForPositionedParents (element, position) {
  ...
  const rpParents = getOffsetParents(element)

  let offsetY = 0
  let offsetX = 0

  for (let i = 1; i < rpParents.length; i++) {
    const rpParent = getBoundingClientRect(rpParents[i])
    const rpChild = getBoundingClientRect(rpParents[i - 1])

    offsetY = offsetY + (rpChild.top - rpParent.top)
    offsetX = offsetX + (rpChild.left - rpParent.left)
  }
...
function adjustForScrollParents (element, position) {
  let { top, left } = position
  ...
  for (let i = 1; i < scrollParents.length; i++) {
    offsetY = offsetY + (scrollParents[i].scrollTop - scrollParents[i - 1].scrollTop)
    offsetX = offsetX + (scrollParents[i].scrollLeft - scrollParents[i - 1].scrollLeft)
  }
