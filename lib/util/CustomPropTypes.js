...
export default {
  ...
  spacing (props, propName) {
    ...
    const propValues = props[propName].split(' ')
    ...
    if (valuesLength > 0 && valuesLength < 5) {
      for (let i = 0; i < valuesLength; i++) {
        ...
        if (valueIndex === -1) {
          return new Error(
            `${propValues[i]} is not a valid spacing value`
          )
        }
      }
    } else {
      return new Error(
        `Please enter between one and four spacing values (you entered ${valuesLength}).`
      )
    }
  },
