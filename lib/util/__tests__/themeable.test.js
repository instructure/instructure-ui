...
describe('@themeable', function () {
  ...
  it('applies theme css variables to the root node', function () {
    ...
    expect(component.getComputedStyle().getPropertyValue('--ThemeableComponent-textColor'))
      .to.equal('purple')
  })

  it('updates css variables when props are updated', function (done) {
    const theme = {
      textColor: 'purple'
    }
  ...
  it('injects styles into the document', function () {
    ...
    const styles = component.getComputedStyle()
