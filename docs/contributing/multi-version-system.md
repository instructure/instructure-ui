...
└── canvas/
    ...
    └── components.ts     # { [componentId]: (semantic) => ({ ... }) }
```

Folders are named after the theme's `key`, so `canvasHighContrast` writes to `canvas-high-contrast/`. That's the string a component looks its frozen theme up with at runtime.

`components.ts` holds every component the version folder owns, not just the top one. `Menu` v2 also ships `Menu.Item`, `Menu.Group`, and `Menu.Separator`, so all four get frozen together and all four sets of token reads count toward what the other files keep.
...
The second argument is `useTokensFrom`, which stays `null`. Function components pass the same object to `useStyleNew` as `frozenTheme`.
...
`useTokensFrom` doesn't work with a frozen theme. A snapshot only covers the components in one version folder, so there's nothing in it to borrow from, and falling back to the live theme would reintroduce the drift freezing exists to stop. `withStyleNew` and `useStyleNew` throw when they get both. Give the component its own tokens and freeze those instead.
