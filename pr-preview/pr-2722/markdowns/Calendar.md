...
| Calendar | renderNextMonthButton | `Renderable<{ targetMonthSrLabel: string }>` | No | - | A button to render in the navigation header. When passed as a function, receives `{ targetMonthSrLabel }` — a pre-formatted screen reader label for the target month (e.g. "January 2024"). The recommendation is to compose it with the [IconButton](Button) component by setting the `size` prop to `small`, `withBorder` and `withBackground` to `false`, and setting `renderIcon` to [IconArrowOpenEnd](icons). |
...
| Calendar | selectedLabel | `string` | Yes | - | Screen reader label appended to the date label when the day is selected. Used to announce the selected state to assistive technologies. |
...
| Calendar.Day | selectedLabel | `string` | Yes | - | Screen reader label appended to the date label when the day is selected. Used to announce the selected state to assistive technologies. |
...
import { Calendar } from '@instructure/ui-calendar/v11_7'
```
