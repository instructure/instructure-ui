...
| FormField | labelId | `string` | No | - | id for the label element, so a single form control can reference just the label text via `aria-labelledby` (keeping messages out of its accessible name) |
...
| FormField | margin | `Spacing` | No | - | Valid values are `0`, `none`, `auto`, and Spacing token values, see https://instructure.design/layout-spacing. Apply these values via familiar CSS-like shorthand. For example, `margin="general.spaceMd auto"`. |
...
| FormField.FormFieldLayout | as | `AsElementType` | No | - | the element type to render as |
...
| FormField.FormFieldLayout | labelId | `string` | No | - | id for the label element, so a single form control can reference just the label text via `aria-labelledby` (keeping messages out of its accessible name) |
...
| FormField.FormFieldLayout | margin | `Spacing` | No | - | Valid values are `0`, `none`, `auto`, and Spacing token values, see https://instructure.design/layout-spacing. Apply these values via familiar CSS-like shorthand. For example, `margin="general.spaceMd auto"`. |
...
import { FormField } from '@instructure/ui-form-field/v11_7'
```
