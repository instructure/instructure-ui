...
| Calendar | renderNextMonthButton | `Renderable<{ targetMonthSrLabel: string }>` | No | - | A button to render in the navigation header. The recommendation is to compose it with the [IconButton](IconButton) component by setting the `size` prop to `small`, `withBorder` and `withBackground` to `false`, and setting `renderIcon` to [IconArrowOpenEnd](icons). |
| Calendar | renderPrevMonthButton | `Renderable<{ targetMonthSrLabel: string }>` | No | - | A button to render in the navigation header. The recommendation is to compose it with the [IconButton](Button) component by setting the `size` prop to `small`, `withBorder` and `withBackground` to `false`, and setting `renderIcon` to [IconArrowOpenStart](icons). |
