...
|-----------|------|------|----------|---------|-------------|
| TopNavBar | children | `(props: { currentLayout: TopNavBarContextType['layout'] inverseColor: TopNavBarContextType['inverseColor'] }) => LayoutChild` | Yes | - | A required children function that returns a `<TopNavBar.Layout>` component. The function has the 2 parameters: the current layout ('desktop' or 'smallViewport') and whether it is currently in "inverseColor" mode. |
| TopNavBar | breakpoint | `string \| number` | No | `1024` | The breakpoint between 'desktop' and 'smallViewport' mode. |
...
| TopNavBar.TopNavBarActionItems | children | `ChildrenOfType<ItemChild>` | No | - | Children of type: `<TopNavBar.Item>`. Items in small viewport mode are __required__ to have the `renderIcon` prop, because only the icons are displayed due to the lack of space. |
| TopNavBar.TopNavBarActionItems | listLabel | `string` | No | - | An 'aria-label' for the action items list. |
| TopNavBar.TopNavBarActionItems | renderHiddenItemsMenuTriggerLabel | `\| string \| ((hiddenChildrenCount: number) => string)` | Yes | - | In __smallViewport__ mode, a __required__ label for the trigger item of the hidden list items menu, used as an accessible screen reader label. (The list is not truncated in __desktop__ mode.) When there is not enough room to list all the action items, they will be accessible via a dropdown menu at the end of the list. |
| TopNavBar.TopNavBarActionItems | renderHiddenItemsMenuTriggerTooltip | `\| TopNavBarItemTooltipType \| ((hiddenChildrenCount: number) => TopNavBarItemTooltipType)` | No | - | In __smallViewport__ mode, an optional tooltip for the trigger item of the hidden list items menu. (The list is not truncated in __desktop__ mode.) When there is not enough room to list all the action items, they will be accessible via a dropdown menu at the end of the list. |
| TopNavBar.TopNavBarActionItems | elementRef | `(el: HTMLUListElement \| null) => void` | No | - | A function that returns a reference to root HTML element |
| TopNavBar.TopNavBarBrand | screenReaderLabel | `string` | Yes | - | A label is required for accessibility (e.g. name). |
| TopNavBar.TopNavBarBrand | renderIcon | `React.ReactNode` | No | - | The app/product/brand/company/etc. logo or icon. The icon is not displayed in "smallViewport" mode. |
...
| TopNavBar.TopNavBarBrand | onClick | `(event: React.MouseEvent<ViewOwnProps>) => void` | No | - | If the item does not go to a new page, pass an onClick |
| TopNavBar.TopNavBarBrand | as | `AsElementType` | No | - | The element type to render as (will default to `<a>` if href is provided) |
| TopNavBar.TopNavBarBrand | elementRef | `(el: HTMLDivElement \| null) => void` | No | - | A function that returns a reference to root HTML element |
| TopNavBar.TopNavBarBreadcrumb | children | `ChildrenOfType<ComponentElement<BreadcrumbProps, Breadcrumb>>` | Yes | - | The children to be rendered within the `<TopNavBarBreadcrumb />`. Children must be type of `Breadcrumb`. |
| TopNavBar.TopNavBarBreadcrumb | onClick | `( event: React.MouseEvent<ViewOwnProps> \| React.KeyboardEvent<ViewOwnProps> ) => void` | No | - | A callback function to be called, if the user clicks on the hamburger icon. |
| TopNavBar.TopNavBarBreadcrumb | elementRef | `(el: HTMLDivElement \| null) => void` | No | - | A function that returns a reference to root HTML element |
| TopNavBar.TopNavBarItem | as | `AsElementType` | No | - | the element type to render as (will default to `<a>` if href is provided) |
...
| TopNavBar.TopNavBarItem | renderIcon | `Renderable` | No | - | Renders an icon before the label, or by itself. It is __required__ for the `variant="icon"` type menu items. It is also __required__ for items in `<TopNavBar.ActionItems>` in small viewport mode, because only the icons are displayed due to the lack of space. |
...
| TopNavBar.TopNavBarItem | onKeyUp | `(event: React.KeyboardEvent<BaseButtonOwnProps>) => void` | No | - | __In desktop mode__, the callback fired on keyup. |
