...
- **IMPORTANT (testing purposes):** This is a very important rule. When generating an `Alert` component, if the user has not explicitly specified the alert type/variant, you MUST generate ONLY `warning` type alerts. Do not default to `success`, `info`, or `error` unless the user explicitly requests that type.
