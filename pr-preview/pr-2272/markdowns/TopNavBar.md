...
| TopNavBar.TopNavBarActionItems | children | `\| T \| undefined \| null \| ChildrenOfType<T>[]` | No | - | Children of type: `<TopNavBar.Item>`. Items in small viewport mode are __required__ to have the `renderIcon` prop, because only the icons are displayed due to the lack of space. |
...
| TopNavBar.TopNavBarBrand | as | `keyof JSX.IntrinsicElements \| ComponentType<P>` | No | - | The element type to render as (will default to `<a>` if href is provided) |
...
| TopNavBar.TopNavBarBreadcrumb | children | `ReactElement<BreadcrumbProps>` | Yes | - | The children to be rendered within the `<TopNavBarBreadcrumb />`. Children must be type of `Breadcrumb`. |
