...
| FileDrop | shouldEnablePreview | `bool` | No | `false` | Flag to use window.URL.createObjectURL for each dropped file and pass it through file.preview |
| FileDrop | shouldAllowMultiple | `bool` | No | `false` | Flag to allow multiple files to drop at once |
| FileDrop | shouldAllowRepeats | `bool` | No | `true` | Flag to allow upload of the same file more than once |
