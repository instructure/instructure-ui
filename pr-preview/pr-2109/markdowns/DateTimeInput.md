...
|-----------|------|------|----------|---------|-------------|
| DateTimeInput | description | `ReactReactNode` | Yes | - | The label over the composite `DateTimeInput` component |
| DateTimeInput | dateRenderLabel | `union` | Yes | - | The label over the DateInput |
...
| DateTimeInput | timeRenderLabel | `union` | Yes | - | The label over the time input |
| DateTimeInput | timeStep | `union` | No | `30` | The number of minutes to increment by when generating the allowable time options. |
...
| DateTimeInput | messages | `Array` | No | - | Extra message(s) to be displayed. |
...
| DateTimeInput | layout | `union` | No | `'inline'` | The layout of this component. Vertically stacked, horizontally arranged in 2 columns, or inline (default). See [FormFieldGroup](#FormFieldGroup) for details. |
| DateTimeInput | rowSpacing | `union` | No | `'small'` | Controls the spacing between the inputs when they are in a vertical layout. |
| DateTimeInput | colSpacing | `union` | No | `'medium'` | Controls the spacing between the inputs when they are in a horizontal layout. |
...
| DateTimeInput | onChange | `signature` | No | - | Called when the date-time value has changed. The passed in parameters are: - *event*: The triggering event (which may be from the underlying `DateInput` or `TimeSelect`) - *isoValue*: The new date value in ISO8601 format, undefined if its invalid |
| DateTimeInput | dateInputRef | `signature` | No | - | The HTML `input` element where the date is entered. |
| DateTimeInput | timeInputRef | `signature` | No | - | The HTML `input` element where the time is entered. |
| DateTimeInput | onBlur | `signature` | No | - | onBlur event handler for when focus leaves DateTimeInput. Does not fire when focus moves between DateInput and TimeSelect within the component |
| DateTimeInput | disabledDates | `union` | No | - |  |
| DateTimeInput | disabledDateTimeMessage | `union` | No | - | Error message shown to the user if they enter a date that is disabled. If not specified the component will show the `invalidDateTimeMessage`. |
