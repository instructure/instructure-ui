...
| TimeSelect | defaultToFirstOption | `bool` | No | `false` | Whether to default to the first option when `defaultValue` hasn't been specified. |
| TimeSelect | value | `custom` | No | - | An ISO 8601 formatted date string representing the current selected value. If defined, the component will act controlled and will not manage its own state. |
| TimeSelect | defaultValue | `custom` | No | - | An ISO 8601 formatted date string to use if `value` isn't provided. |
...
| TimeSelect | isRequired | `bool` | No | `false` |  |
| TimeSelect | isInline | `bool` | No | `false` | Whether the input is rendered inline with other elements or if it is rendered as a block level element. |
...
| TimeSelect | placement | `custom` | No | `'bottom stretch'` | The placement of the options list. |
| TimeSelect | constrain | `custom` | No | `'window'` | The parent in which to constrain the placement. |
| TimeSelect | mountNode | `custom` | No | - | An element or a function returning an element to use mount the options list to in the DOM (defaults to `document.body`) |
...
import { TimeSelect } from '@instructure/ui-time-select/es/TimeSelect/index'
```
