...
| ProgressBar | shouldAnimate | `bool` | No | `false` | Whether the change of value should have a transition |
| ProgressBar | margin | `string` | No | - | Valid values are `0`, `none`, `auto`, `xxx-small`, `xx-small`, `x-small`, `small`, `medium`, `large`, `x-large`, `xx-large`. Apply these values via familiar CSS-like shorthand. For example: `margin="small auto large"`. |
...
import { ProgressBar } from '@instructure/ui-progress/es/ProgressBar/index'
```
