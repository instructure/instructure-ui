...
| ProgressBar | size | `union` | No | `'medium'` | Control the height of the progress bar |
| ProgressBar | valueMax | `number` | No | `100` | Maximum value (defaults to 100) |
| ProgressBar | valueNow | `number` | No | `0` | Receives the progress of the event |
| ProgressBar | formatScreenReaderValue | `signature` | No | `({ valueNow, valueMax }: Values) =>
`${valueNow} / ${valueMax}`` | A function for formatting the text provided to screen readers via `aria-valuenow` |
| ProgressBar | renderValue | `union` | No | - | A function to format the displayed value. If null the value will not display. Takes `valueNow` and `valueMax` as parameters. |
| ProgressBar | color | `union` | No | `'primary'` | Controls the overall color scheme of the component |
| ProgressBar | meterColor | `union` | No | `({ valueNow, valueMax }: Values) =>
...
| ProgressBar | elementRef | `signature` | No | - | Provides a reference to the component's root HTML element |
| ProgressBar | as | `union` | No | `'div'` | Set the element type of the component's root |
