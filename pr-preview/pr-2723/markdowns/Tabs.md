...
Set `activationMode="manual"` to separate the two: the arrow keys, Home
and End only move focus, and Enter or Space selects the focused tab.

Use manual activation for expensive panels. This is also important for VoiceOver, which reserves arrow keys for its cursor, while Enter and Space work across screen readers.
...
| Tabs | activationMode | `'auto' \| 'manual'` | No | `'auto'` | With `auto`, arrow keys select a tab as soon as focus reaches it. With `manual`, arrow keys only move focus and Enter or Space selects the focused tab. Screen readers that reserve the arrow keys for their own cursor (VoiceOver) need `manual`. |
...
| Tabs.Tab | isTabbable | `boolean` | No | - |  |
...
| Tabs.Tab | onFocus | `( event: React.FocusEvent<ViewOwnProps>, tabData: { index: number; id: string } ) => void` | No | - |  |
